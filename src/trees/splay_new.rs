...
impl<D: Data, BT> Drop for SplayTree<D, BT>
where
    BT: super::basic_tree_trait::BasicTreeTrait<D, ()>,
    for<'a> &'a mut BT: SomeTreeRef<D>, 
{
    fn drop(&mut self) {
        todo!()
    ...
    fn split_right(&mut self) -> Option<SplayTree<D>> {
        ...
        match side {
            Side::Left => {
                ...
                Some(SplayTree { tree, phantom: PhantomData })
            }
            Side::Right => {
